...
function parseEmbeddedValue(val) {
...
function getScoreTypeHelper(name) {
...
function getScoreType(question) {
...
function getMaxScoreForQuestion(question) {
...
function calculateFinalScore(survey) {
    ...
    valueNames.forEach(name => {
        ...
        if (currentQuestionType === 2) {
            ...
            maxMitigationScore += getMaxScoreForQuestion((currentQuestion));
        }
    });
...
const store = {
    ...
    getters: {
        ...
        plainData: state => {
            if (state.result === undefined)
                return {};
            if (state.result.data === undefined)
                return {};
        ...
        resultDataSections: state => {
            ...
            var projectResults = [];
            var riskResults = [];
            var mitigationResults = [];
            var mitigationResultsYes = [];
